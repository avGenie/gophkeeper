...
// UpdateLoginPassword Updates login-password data for user
func (s *GRPCServer) UpdateLoginPassword(ctx context.Context, loginPasswordData *pb.LoginPasswordData) (*emptypb.Empty, error) {
	...
	err = s.storage.UpdateLoginPasswordData(ctx, loginPass, userID)
	if err != nil {
		zap.S().Error("failed to update login-password data", zap.Error(err))
...
// UpdateText Updates text data for user
func (s *GRPCServer) UpdateText(ctx context.Context, textData *pb.TextData) (*emptypb.Empty, error) {
	userID, err := usecase.GetUserIDFromContext(ctx)
	if err != nil {
		zap.S().Error("error while getting user id from context", zap.Error(err))
	...
	err = s.storage.UpdateTextData(ctx, data, userID)
	if err != nil {
...
// UpdateCard Updates card data for user
func (s *GRPCServer) UpdateCard(ctx context.Context, inputData *pb.CardData) (*emptypb.Empty, error) {
	userID, err := usecase.GetUserIDFromContext(ctx)
	if err != nil {
		zap.S().Error("error while getting user id from context", zap.Error(err))
	...
	err = s.storage.UpdateCardData(ctx, data, userID)
	if err != nil {
		if errors.Is(err, storage.ErrDataExists) {
			zap.S().Info("failed to save login-password data", zap.Error(err))
